...
export default class UploadFormItem extends React.Component {
  ...
  onDeleteFile = (fileIndex) => {
    const tempArr = this.props.uploadedFileList
  ...
  beforeUpload = (file) => {
    if (file.size > 0) {
      const newList = this.props.newFileList
      newList.push(file)
  ...
  onRemove = (file) => {
    ...
    const newList = this.props.newFileList.slice()
  ...
  render() {
    ...
    return (
      <div>
        <FormItem
          label={
            <span className={cx('uploadedItem_list_title')}>
              已上传文件：{resetBtn}
            </span>
          }
        >
          {uploadedFileList && uploadedFileList.length > 0 ? (
            uploadedFileList.map((item, index) => (
              <div className={cx('uploadingFileList')} key={index}>
        ...
        >
          ...
          >
            {uploadBoxLimit !== 0 ? (
              <div className={cx('upload_formItem_place')}>
                <IconFont
                  className={cx('upload_formItem_place_icon')}
                  type='iconuploadBig'
                />
                <h6>点击选择需要上传的文件</h6>
                <p>也可将文件拖拽到虚线内进行上传</p>
