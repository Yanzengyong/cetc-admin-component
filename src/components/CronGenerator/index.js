...
export default class GronGenerator extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (nextProps.dialogVisible && nextProps.initCron) {
      const cronArr = nextProps.initCron.split(' ')
      const newExpress = JSON.parse(JSON.stringify(this.state.express))
  ...
  reverseGenerateCRON = (express) => {
    const newExpressType = JSON.parse(JSON.stringify(this.state.expressType))
  ...
  getType = (expStr, expressType) => {
    if (expStr === '*') return 'everyTime'
      ...
      const newPeriodTime = this.state.periodTime
      const values = expStr.split('-')
      newPeriodTime[expressType] = {
      ...
      const newLoopTime = this.state.loopTime
      const values = expStr.split('/')
      newLoopTime[expressType] = {
      ...
      const newEnumTime = this.state.enumTime
      const values = expStr.split(',')
      ...
      const value = parseInt(expStr.substring(0, 1))
      ...
      newWeekday = {
        weekNum: parseInt(values[0]),
        weekDayNum: parseInt(values[1])
      }
  ...
  onSelectType = (type, expressType) => {
    const expressionTypeState = this.state.expressType
  ...
  getExpressStr = (type, expressType) => {
    switch (type) {
      ...
      case 'lastDay':
        return 'L'
      case 'period':
        return this.expressGenerator(type, this.state.periodTime[expressType])
      case 'loop':
        return this.expressGenerator(type, this.state.loopTime[expressType])
      case 'enum':
        return this.expressGenerator(type, this.state.enumTime[expressType])
      case 'mostRecentWorkDay':
        return this.state.mostRecentWorkDay + 'W'
      case 'lastWeekDay':
        return this.state.lastWeekDay + 'L'
      case 'weekday':
        return this.expressGenerator(type, this.state.weekday)
      case 'optional':
        return ''
      default:
    }
  ...
  expressGenerator = (type, value) => {
    ...
    switch (type) {
      ...
      case 'loop':
        return value ? value.startTime + '/' + value.period : ''
      case 'enum':
        return getEnumString(value)
      case 'weekday':
        return value ? value.weekNum + '#' + value.weekDayNum : ''
      default:
        return '*'
    }
  }
  ...
  judgeSpecialCRON = (type, expressType) => {
    const expressState = JSON.parse(JSON.stringify(this.state.express))
  ...
  onSelectPeriod = (num, expressType, numType) => {
    const period = JSON.parse(JSON.stringify(this.state.periodTime))
    if (numType === 'max') {
  ...
  onSelectLoop = (num, expressType, numType) => {
    const loop = JSON.parse(JSON.stringify(this.state.loopTime))
  ...
  onSelectEnum = (values, expressType) => {
    const enumObj = JSON.parse(JSON.stringify(this.state.enumTime))
  ...
  onSelectWeekDay = (value, type) => {
    const weekday = JSON.parse(JSON.stringify(this.state.weekday))
    if (type === 'weekNum') {
  ...
  render() {
    ...
    const checkboxGroup = (num, colNum, type) => {
      ...
      const rowNum =
        num % colNum === 0 ? parseInt(num / colNum) : parseInt(num / colNum) + 1

      for (let row = 0; row < rowNum; row++) {
        const startNum = row * colNum
        let endNum = (row + 1) * colNum

        if (row === rowNum - 1 && num % colNum > 0) {
          endNum = num
        }
        checkboxArray.push(generateRow(startNum, endNum, type))
      }
      ...
      >
        <Tab shape='pure' className='cron_generator_container'>
          <Tab.Item title='秒' key={1}>
            <Radio.Group
              value={expressType.second}
              itemDirection='ver'
              className='cron_info'
              onChange={(type) => this.onSelectType(type, 'second')}
            >
              ...
              <Radio value='period'>
                ...
                <NumberPicker
                  min={1}
                  max={58}
                  value={periodTime.second.min}
                  onChange={(num) => this.onSelectPeriod(num, 'second', 'min')}
                />
                {spaceDOM(2)}到{spaceDOM(2)}
                <NumberPicker
                  min={2}
                  max={59}
                  value={periodTime.second.max}
                  defaultValue={2}
                  onChange={(num) => this.onSelectPeriod(num, 'second', 'max')}
                />
                {spaceDOM(2)}秒
              </Radio>
              ...
              <Radio value='loop'>
                ...
                <NumberPicker
                  min={0}
                  max={59}
                  value={loopTime.second.startTime}
                  defaultValue={1}
                  onChange={(num) =>
                    this.onSelectLoop(num, 'second', 'startTime')
                  }
                />
                ...
                <NumberPicker
                  min={1}
                  max={59}
                  value={loopTime.second.period}
                  defaultValue={2}
                  onChange={(num) => this.onSelectLoop(num, 'second', 'period')}
                />
                {spaceDOM(2)}秒执行一次
              </Radio>
              ...
              <Radio value='enum'>
                指定： <br />
                <Checkbox.Group
                  onChange={(values) => this.onSelectEnum(values, 'second')}
                  value={enumTime.second}
                >
          ...
          <Tab.Item title='分钟' key={2}>
            <Radio.Group
              value={expressType.minute}
              itemDirection='ver'
              className='cron_info'
              onChange={(type) => this.onSelectType(type, 'minute')}
            >
              <Radio value='everyTime'>每秒</Radio>
              ...
              <Radio value='period'>
                ...
                <NumberPicker
                  min={1}
                  max={58}
                  value={periodTime.minute.min}
                  onChange={(num) => this.onSelectPeriod(num, 'minute', 'min')}
                />
                {spaceDOM(2)}到{spaceDOM(2)}
                <NumberPicker
                  min={2}
                  max={59}
                  value={periodTime.minute.max}
                  defaultValue={2}
                  onChange={(num) => this.onSelectPeriod(num, 'minute', 'max')}
                />
                {spaceDOM(2)}分钟
              </Radio>
              ...
              <Radio value='loop'>
                ...
                <NumberPicker
                  min={0}
                  max={59}
                  value={loopTime.minute.startTime}
                  defaultValue={1}
                  onChange={(num) =>
                    this.onSelectLoop(num, 'minute', 'startTime')
                  }
                />
                ...
                <NumberPicker
                  min={1}
                  max={59}
                  value={loopTime.minute.period}
                  defaultValue={2}
                  onChange={(num) => this.onSelectLoop(num, 'minute', 'period')}
                />
                {spaceDOM(2)}分钟执行一次
              </Radio>
              ...
              <Radio value='enum'>
                指定： <br />
                <Checkbox.Group
                  onChange={(values) => this.onSelectEnum(values, 'minute')}
                  value={enumTime.minute}
                >
          ...
          <Tab.Item title='小时' key={3}>
            <Radio.Group
              value={expressType.hour}
              itemDirection='ver'
              className='cron_info'
              onChange={(type) => this.onSelectType(type, 'hour')}
            >
              ...
              <Radio value='period'>
                ...
                <NumberPicker
                  min={1}
                  max={58}
                  value={periodTime.hour.min}
                  onChange={(num) => this.onSelectPeriod(num, 'hour', 'min')}
                />
                {spaceDOM(2)}到{spaceDOM(2)}
                <NumberPicker
                  min={2}
                  max={59}
                  value={periodTime.hour.max}
                  defaultValue={2}
                  onChange={(num) => this.onSelectPeriod(num, 'hour', 'max')}
                />
                {spaceDOM(2)}小时
              </Radio>
              ...
              <Radio value='loop'>
                ...
                <NumberPicker
                  min={0}
                  max={59}
                  value={loopTime.hour.startTime}
                  defaultValue={1}
                  onChange={(num) =>
                    this.onSelectLoop(num, 'hour', 'startTime')
                  }
                />
                {spaceDOM(2)}分钟开始，每{spaceDOM(2)}
                <NumberPicker
                  min={1}
                  max={59}
                  value={loopTime.hour.period}
                  defaultValue={2}
                  onChange={(num) => this.onSelectLoop(num, 'hour', 'period')}
                />
                {spaceDOM(2)}小时执行一次
              </Radio>
              ...
              <Radio value='enum'>
                指定： <br />
                <Checkbox.Group
                  onChange={(values) => this.onSelectEnum(values, 'hour')}
                  value={enumTime.hour}
                >
          ...
          <Tab.Item title='日' key={4}>
            <Radio.Group
              value={expressType.date}
              itemDirection='ver'
              className='cron_info'
              onChange={(type) => this.onSelectType(type, 'date')}
            >
              ...
              <Radio value='period'>
                ...
                <NumberPicker
                  min={1}
                  max={30}
                  value={periodTime.date.min}
                  onChange={(num) => this.onSelectPeriod(num, 'date', 'min')}
                />
                {spaceDOM(2)}到{spaceDOM(2)}
                <NumberPicker
                  min={2}
                  max={31}
                  value={periodTime.date.max}
                  defaultValue={2}
                  onChange={(num) => this.onSelectPeriod(num, 'date', 'max')}
                />
                {spaceDOM(2)}天
              </Radio>
              ...
              <Radio value='loop'>
                ...
                <NumberPicker
                  min={0}
                  max={31}
                  value={loopTime.date.startTime}
                  defaultValue={1}
                  onChange={(num) =>
                    this.onSelectLoop(num, 'date', 'startTime')
                  }
                />
                ...
                <NumberPicker
                  min={1}
                  max={31}
                  value={loopTime.date.period}
                  defaultValue={2}
                  onChange={(num) => this.onSelectLoop(num, 'date', 'period')}
                />
                {spaceDOM(2)}天执行一次
              </Radio>
              ...
              <Radio value='enum'>
                指定： <br />
                <Checkbox.Group
                  onChange={(values) => this.onSelectEnum(values, 'date')}
                  value={enumTime.date}
                >
          ...
          <Tab.Item title='月' key={5}>
            <Radio.Group
              value={expressType.month}
              itemDirection='ver'
              className='cron_info'
              onChange={(type) => this.onSelectType(type, 'month')}
            >
              ...
              <Radio value='period'>
                ...
                <NumberPicker
                  min={1}
                  max={30}
                  value={periodTime.month.min}
                  onChange={(num) => this.onSelectPeriod(num, 'month', 'min')}
                />
                {spaceDOM(2)}到{spaceDOM(2)}
                <NumberPicker
                  min={2}
                  max={31}
                  value={periodTime.month.max}
                  defaultValue={2}
                  onChange={(num) => this.onSelectPeriod(num, 'month', 'max')}
                />
                {spaceDOM(2)}月
              </Radio>
              ...
              <Radio value='loop'>
                ...
                <NumberPicker
                  min={0}
                  max={31}
                  value={loopTime.month.startTime}
                  defaultValue={1}
                  onChange={(num) =>
                    this.onSelectLoop(num, 'month', 'startTime')
                  }
                />
                ...
                <NumberPicker
                  min={1}
                  max={31}
                  value={loopTime.month.period}
                  defaultValue={2}
                  onChange={(num) => this.onSelectLoop(num, 'month', 'period')}
                />
                {spaceDOM(2)}月执行一次
              </Radio>
              ...
              <Radio value='enum'>
                指定： <br />
                <Checkbox.Group
                  onChange={(values) => this.onSelectEnum(values, 'month')}
          ...
          <Tab.Item title='周' key={6}>
            <Radio.Group
              value={expressType.week}
              itemDirection='ver'
              className='cron_info'
              onChange={(type) => this.onSelectType(type, 'week')}
            >
              <Radio value='everyTime'>每日</Radio>
              ...
              <Radio value='period'>
                ...
                <NumberPicker
                  min={1}
                  max={30}
                  value={periodTime.week.min}
                  onChange={(num) => this.onSelectPeriod(num, 'week', 'min')}
                />
                {spaceDOM(2)}到星期{spaceDOM(2)}
                <NumberPicker
                  min={2}
                  max={31}
                  value={periodTime.week.max}
                  defaultValue={2}
                  onChange={(num) => this.onSelectPeriod(num, 'week', 'max')}
                />
                {spaceDOM(2)}
              </Radio>
              ...
              <Radio value='weekday'>
                ...
                <NumberPicker
                  min={1}
                  max={4}
                  value={weekday.weekNum}
                  defaultValue={1}
                  onChange={(num) => this.onSelectWeekDay(num, 'weekNum')}
                />
                ...
                <NumberPicker
                  ...
                  defaultValue={1}
                  onChange={(num) => this.onSelectWeekDay(num, 'weekDayNum')}
                />
                {spaceDOM(2)}
              </Radio>
              ...
              <Radio value='enum'>
                指定： <br />
                <Checkbox.Group
                  onChange={(values) => this.onSelectEnum(values, 'week')}
                  value={enumTime.week}
                >
          ...
          <Tab.Item title='年' key={7}>
            <Radio.Group
              value={expressType.year}
              itemDirection='ver'
              className='cron_info'
              onChange={(type) => this.onSelectType(type, 'year')}
            >
              ...
              <Radio value='period'>
                ...
                <NumberPicker
                  min={1970}
                  max={2099}
                  value={periodTime.year.min}
                  onChange={(num) => this.onSelectPeriod(num, 'year', 'min')}
                />
                {spaceDOM(2)}年到{spaceDOM(2)}
                <NumberPicker
                  min={1970}
                  max={2099}
                  value={periodTime.year.max}
                  defaultValue={2}
                  onChange={(num) => this.onSelectPeriod(num, 'year', 'max')}
                />
                {spaceDOM(2)}年
              </Radio>
            </Radio.Group>
